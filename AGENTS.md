...
- Publish from the repository root with `./scripts/publish-docker-images.sh`; it must produce Linux `amd64` and `arm64` manifests.
- Set `MOVA_PUBLISH_BASE_IMAGES=1` only when base images must be deliberately republished.
- Preview releases use an immutable Docker tag such as `richeschiu/mova:1.0.0-preview.2`. After verification, move `preview` and, before 1.0, `latest` to the same manifest.
