...
Mova 当前公开版本为 `1.0.0 Preview`，适合本机、家庭服务器和私人媒体库体验，正式 `1.0` 预计在一个月内发布。快速迭代期间可能出现破坏性数据库结构调整，升级后可能需要重建数据库并重新扫描媒体库。`preview` 始终指向最新预览镜像；需要固定版本时，请使用 `1.0.0-preview.3` 这类不可变标签。
