...
查询参数：
- `library_limit`：可选，返回多少个媒体库分组，默认 `3`；首页会传入当前可访问媒体库数量，用于展示所有有新增内容的媒体库分组
- `item_limit`：可选，每个媒体库返回多少个媒体条目，默认 `8`，最大 `50`
- `limit`：可选，`item_limit` 的别名，适合原生客户端按“每库条数”直传；如果同时传 `item_limit` 和 `limit`，以 `item_limit` 为准
- `days`：可选，只返回最近多少天内新增的媒体条目，默认 `7`，最大 `365`

排序语义：
- 媒体条目按 `media_items.created_at desc, id desc` 排序
- 媒体库分组按各自最近一个媒体条目的 `created_at desc` 排序
- 查询默认只覆盖最近 7 天内新增的媒体条目；没有可展示媒体条目的库不会出现在返回结果里，前端应显示真实空态，而不是用其他列表接口补一个假分组
...
- `total`：该库内符合此接口展示范围的媒体条目总数，不受 `item_limit` 截断影响
