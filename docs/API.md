...
- `scan.job.updated` / `scan.job.finished` 里的 `scan_job` 目前会额外带一个可选 `phase` 字段，当前会使用：
  ...
  - `syncing`：正在写入媒体库
  - `finished`：任务已结束
- `scan.item.updated` 用于扫描中的条目级提示；服务端会先完成本地目录、文件名、资源探测和资源聚合，把本轮需要处理的电影文件或剧集目录组先写入数据库并推送出来；如果同路径文件指纹和本地分析版本都未变化但仍需补 TMDB，会直接复用数据库里的上次本地分析结果，不重新拆名、探测或聚合；随后才逐条访问 TMDB，同一个 `item_key` 会在后续元数据、图片和完成写库后持续更新，当前结构如下：
...
- `stage = discovered` 表示该条目已经完成本地分析并写入媒体库；`stage = completed` 表示该条目已经完成 metadata / 海报覆盖写库。客户端应在这些事件后重拉该库的媒体列表、首页 shelf 和库详情计数，让真实媒体卡先出现、再按条目逐个更新海报
