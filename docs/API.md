...
- 已经成功匹配、同路径文件指纹未变化、本地分析版本未变化、已有 TMDB 绑定且已有本地可见海报的条目不会再次发出 `scan.item.updated`；该事件只覆盖本轮新增、删除后重建、移动改名后新增、大小 / 修改时间变化、本地分析版本过期、仍需重试匹配、从未成功绑定 TMDB、缺少可见海报或仍需把远端图片 URL 缓存成本地文件的资源
- 当前已实现事件：
  - `scan.job.updated`
  - `scan.job.finished`
  - `scan.item.updated`
...
- `scan.job.updated` / `scan.job.finished` 里的 `scan_job` 目前会额外带一个可选 `phase` 字段，当前会使用：
  - `discovering`：正在发现文件
  - `analyzing`：正在做本地目录、文件名、资源探测和扫描展示聚合，不访问 TMDB
  - `enriching`：正在补全元数据和图片
  - `syncing`：正在写入媒体库
  - `finished`：任务已结束
- `scan.item.updated` 用于扫描中的条目级提示；服务端会先完成本地目录、文件名、资源探测和资源聚合，把本轮需要处理的电影文件或剧集目录组先写入数据库并推送出来；如果同路径文件指纹和本地分析版本都未变化但仍需补 TMDB，会直接复用数据库里的上次本地分析结果，不重新拆名、探测或聚合；随后才按聚合组串行访问 TMDB，同一剧集组只做一次剧集 metadata 查询并应用到组内所有集，同一个 `item_key` 会在后续元数据、图片和完成写库后持续更新，当前结构如下：
...
- `stage = discovered` 表示该条目已经完成本地分析并写入媒体库；`stage = completed` 表示该条目已经完成 metadata / 海报覆盖写库。客户端应在这些事件后重拉该库的媒体列表、首页 shelf 和库详情计数，让真实媒体卡先出现、再按聚合组逐个更新海报
...
- 已经成功匹配的路径会先按文件大小和修改时间生成稳定指纹；同路径指纹一致、本地分析版本一致、已有 TMDB 绑定且已有本地可见海报时会跳过拆名、sidecar、`ffprobe`、TMDB / OMDb、图片缓存和数据库 upsert，只保留现有数据。`unmatched` / `failed`、从未成功绑定 TMDB、缺少可见海报、或仍保存远端图片 URL 的条目仍会在后续手动扫描中重试；如果这些条目的文件指纹和本地分析版本未变化，服务端会复用已入库的本地分析结果，仅重试 TMDB 和图片缓存。如果当前没有启用 metadata provider，`skipped` 条目也可按指纹和本地分析版本跳过
