...
说明：
- 如果创建的是启用状态的媒体库，服务会在创建成功后自动启动该库的文件 watcher，并触发第一次后台扫描
- 启用中的媒体库还会在服务进程内定时做一次后台路径校准，作为 watcher 的兜底
- 如果创建时 `is_enabled = false`，则不会自动扫描，后续需要手动调用 `POST /api/libraries/{id}/scan`
...
- 除了手动扫描外，已启用媒体库还会通过 watcher 自动处理常见新增、删除、修改、改名和目录变化；手动扫描主要作为显式全库校准入口
- 服务进程还会定时执行一次后台路径校准，只对齐文件路径和展示列表，不会对未变化文件重跑整库 metadata enrichment
...
- 查询单个媒体条目的主演员列表
- 只返回当前数据库里已经缓存好的演员列表
- 不会在这个详情页请求里直接触发远端演员拉取
- 演员信息会在扫库和手动 metadata 刷新阶段预取或刷新
...
- 后续该媒体条目的演员缓存和剧集 outline 会优先按这个精确 TMDB ID 拉取，而不是再走模糊搜索
...
- 如果该媒体条目之前已经通过 `POST /api/media-items/{id}/metadata-match` 绑定过精确 TMDB 条目，后续演员缓存和剧集 outline 仍会沿用该绑定
