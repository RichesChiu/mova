...
只有直接标题阶段完全没有候选时，才调用 alternative titles。别名验证仍按完整相等优先、数字副标题兼容次之，不产生分数。最多验证 40 个候选，避免无界 N+1 请求。无年份查询按 5.3 的首屏精准标题优先、provider 顺序兜底规则处理，不调用 alternative titles。
...
| 状态 | 原因 | 含义 |
| --- | --- | --- |
| `matched` | `null` | 按当前年份策略选定 TMDB 身份并完成规范字段写入 |
| `unmatched` | `no_remote_match` | 指定类型中没有符合当前年份策略的候选 |
