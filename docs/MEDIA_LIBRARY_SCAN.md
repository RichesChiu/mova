...
- 文件系统、`ffprobe`、TMDB、OMDb 和图片下载不得放在数据库事务中。
...
发现阶段不执行 sidecar 解析、`ffprobe`、TMDB、OMDb 或图片下载。
...
series group key 优先使用明确的剧集容器目录与规范化剧名，避免不同目录下的同名作品误合并。

### 8.3 分组约束

- group key 在同一个 scan job 内唯一且稳定。
- 年份是远端匹配条件，不是剧集跨季拆组条件。
- 同一物理文件只能属于一个扫描组。
...
NFO 可以提供标题、年份、简介、季集坐标和 provider ID。NFO provider ID 必须按本地确定的唯一 media type 校验。
