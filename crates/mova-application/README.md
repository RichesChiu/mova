...
- 远端补全阶段才按本地聚合组串行访问 TMDB 做类型确认、metadata、海报和本地图片缓存；同一剧集组只做一次剧集 metadata 查询，再把 TMDB 标题和剧集级海报应用到组内所有集；中文元数据库里，如果季目录上一级或无季目录时的直接父级目录提供中文剧名，会把该中文剧名作为补充候选，覆盖 `莎拉的真伪人生(2026)/The.Art.of.Sarah.S01E01.mkv` 这类平铺剧集目录；成功后标记 `metadata_status = matched` 并立即覆盖写库，失败或从未成功访问过 TMDB 的条目会在后续手动扫描中重试
