...
Web 主页以媒体库为第一层级：展示继续观看、简短的 `你的库` 摘要，以及由服务端按库聚合的最近 7 天最新添加内容，而不是前端把各库按标题排序后的列表再混合拼出来。
...
服务器设置中的媒体库卡片为描述固定保留两行高度，只展示紧凑的扫描状态；根路径超长时单行省略，悬浮后可查看完整路径。编辑、扫描和删除统一收进与首页库卡片一致的三点菜单。
...
当前仍处于 pre-1.0 MVP 预览版阶段，数据库 schema 变化时可能需要重建 `data/postgres/`。schema 变更仍会直接落在 `migrations/0001_init.sql` 中，所以拉取涉及 schema 的更新后，开发数据库可能需要重建。
