...
其中 `scan.job.updated` 在发现文件阶段会先节流并合并进度写库 / 广播，避免大库扫描时按文件数量打满 SSE；发现阶段结束仍会强制写入最终文件数。扫描随后进入 `analyzing` 本地分析阶段，只做目录、文件名、sidecar、`ffprobe` 和扫描展示聚合，不访问 TMDB；本地分析完成后，服务端会先把聚合出的电影文件或剧集组写入数据库并通过 `scan.item.updated` 推送给前端，再进入 `enriching` 阶段按聚合组串行访问 TMDB。同一剧集组只做一次剧集 metadata 查询，拿到 TMDB 标题和剧集级海报后应用到组内所有集；每个组完成后立即覆盖写库并推送更新。事件会携带当前可用的 `year`、`overview` 和 `metadata_status`；`poster_path` / `backdrop_path` 只在服务端确认是浏览器可访问 URL 时才带出，避免缓存完成前让前端加载本地路径或失效远端图。当 `stage = completed` 时，该组已经完成远端 metadata / 海报覆盖写库，Web 端会立即重拉该库媒体列表、首页 shelf 和库详情计数，让真实媒体卡也跟着逐个更新。已经成功匹配、文件大小 / 修改时间指纹未变化、并且本地分析版本未变化的条目通常不会再次发出 `scan.item.updated`；但如果缺少 TMDB 绑定、缺少可见海报，或仍保存远端图片 URL 而不是本地缓存文件，会复用数据库里的本地分析结果重新进入补全流程。只有本轮新增、移动改名后新增、内容变化、本地分析版本过期、仍需重试匹配或仍需补齐本地图片缓存的资源才进入条目级进度。剧集远端查询会先从文件名解析 `SxxExx` 前的剧名和年份；如果季目录结构明确，服务端会从季目录上一级提取干净剧名和年份作为补充搜索线索，覆盖类似 `Taxi.Driver.S01E01.mkv`、`Study Group S01E01 - 第 1 集.mkv`、`The.BeautyS01E01.mkv` 的场景。中文元数据库里，如果季目录上一级提供中文剧名，会优先把中文剧名作为 TMDB 查询候选，再回退到英文文件名；已经入库但标题仍是英文、且路径里能识别中文剧名的 matched 剧集条目，也会在后续扫描中重新进入 metadata 流程，避免英文文件名先命中导致卡片标题长期偏英文。没有本地季集号的文件会先做 TMDB movie / tv 类型确认，远端更像剧集但本地无季集号、或远端没有命中时会写入明确的 `metadata_status` / `metadata_failure_reason` 并保留为前端 Other 复核项；TMDB 未启用时则标记为 `skipped`，不阻断本地媒体展示。所有事件都会先经过 `is_visible_to(&UserProfile)` 做库级可见性过滤，再转换成 SSE。

另外，扫描现在先做轻量文件清单和同路径 `scan_hash` / `local_analysis_version` 比对；已匹配且未变化、已经有 TMDB 绑定、并且已经有本地可见海报的路径不会重新跑拆名、sidecar、`ffprobe`、TMDB / OMDb、图片缓存或数据库 upsert。新增、变更或本地分析版本过期的路径会重新做本地分析和聚合；文件指纹与本地分析版本都未变化但未匹配成功、从未成功绑定 TMDB、缺少可见海报或仍保留远端图片 URL 的路径，会从数据库恢复上次本地分析结果，跳过拆名、sidecar、`ffprobe` 和重新聚合，只逐组串行补 metadata/海报、立即 upsert，然后继续处理下一组。缺失路径会在最后统一删除。
