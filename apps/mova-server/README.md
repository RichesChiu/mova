...
其中 `scan.job.updated` 在发现文件阶段会先节流并合并进度写库 / 广播，避免大库扫描时按文件数量打满 SSE；发现阶段结束仍会强制写入最终文件数。`scan.item.updated` 现在不再只是“文件级事件”：电影通常仍按文件路径发出，剧集会按显式文件名或明确季目录树发出一张占位卡，避免前端在元数据拉取前先看到被打散的单集文件。事件会携带当前可用的 `year`、`overview` 和 `metadata_status`；`poster_path` / `backdrop_path` 只在服务端确认是浏览器可访问 URL 时才带出，避免缓存完成前让前端加载本地路径或失效远端图。当 `stage = completed` 时，该组已经写入数据库，Web 端会立即重拉该库媒体列表、首页 shelf 和库详情计数，让真实媒体卡也跟着逐个出现。已经成功匹配且文件大小 / 修改时间指纹未变化的条目不会再次发出 `scan.item.updated`，只有本轮新增、移动改名后新增、内容变化或仍需重试匹配的资源才进入条目级进度。剧集远端查询会先从文件名解析 `SxxExx` 前的剧名和年份；如果季目录结构明确，服务端会从季目录上一级提取干净剧名和年份作为补充搜索线索，覆盖类似 `Taxi.Driver.S01E01.mkv`、`Study Group S01E01 - 第 1 集.mkv`、`The.BeautyS01E01.mkv` 的场景。中文元数据库里，如果季目录上一级提供中文剧名，会优先把中文剧名作为 TMDB 查询候选，再回退到英文文件名；已经入库但标题仍是英文、且路径里能识别中文剧名的 matched 剧集条目，也会在后续扫描中重新进入 metadata 流程，避免英文文件名先命中导致卡片标题长期偏英文。没有本地季集号的文件会先做 TMDB movie / tv 类型确认，远端更像剧集但本地无季集号、或远端没有命中时会写入明确的 `metadata_status` / `metadata_failure_reason` 并保留为前端 Other 复核项；TMDB 未启用时则标记为 `skipped`，不阻断本地媒体展示。所有事件都会先经过 `is_visible_to(&UserProfile)` 做库级可见性过滤，再转换成 SSE。

另外，扫描现在先做轻量文件清单和 `scan_hash` 比对；已匹配且未变化的路径不会重新跑 `ffprobe`、TMDB / OMDb、图片缓存或数据库 upsert。缺失路径会在最后统一删除，新增、变更和未匹配成功的路径会逐组串行探测、补 metadata/海报、立即 upsert，然后继续处理下一组，避免健康条目被单条异常数据拖住，也避免前端只能等整库结束后一次性看到海报。
