...
其中 `scan.job.updated` 在发现文件阶段会先节流并合并进度写库 / 广播，避免大库扫描时按文件数量打满 SSE；发现阶段结束仍会强制写入最终文件数。`scan.item.updated` 现在不再只是“文件级事件”：电影通常仍按文件路径发出，剧集会按显式文件名或明确季目录树发出一张占位卡，避免前端在元数据拉取前先看到被打散的单集文件。剧集远端查询会优先使用文件名解析出的标题；如果季目录结构明确，服务端会从季目录上一级提取干净剧名和年份作为补充搜索线索，覆盖类似 `Taxi.Driver.S01E01.mkv`、`Study Group S01E01 - 第 1 集.mkv`、`The.BeautyS01E01.mkv` 的场景。没有本地季集号的文件会先做 TMDB movie / tv 类型确认，远端更像剧集但本地无季集号、或远端没有命中时会写入明确的 `metadata_status` / `metadata_failure_reason` 并保留为前端 Other 复核项；TMDB 未启用时则标记为 `skipped`，不阻断本地媒体展示。所有事件都会先经过 `is_visible_to(&UserProfile)` 做库级可见性过滤，再转换成 SSE。
