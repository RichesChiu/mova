...
- 演员信息会在扫库和手动 metadata 刷新时预取入库；详情页主体与演员区都只读取本地缓存，不再被远端 cast 请求阻塞
...
- 剧集片头区间当前会在扫描写库后做一轮 season 级自动检测，依赖运行环境同时具备 `ffmpeg` 和 `python3`
...
- 启动后先登录或初始化首个管理员
- 初始化出来的首个管理员会被视为 `Primary Admin`，可以管理普通管理员；其他管理员仍然可以管理媒体库和普通成员，但不能修改或删除平级管理员
- 在设置页里创建媒体库、编辑库、管理用户
